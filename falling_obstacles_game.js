...
class Player{
    ...
    move = function(){
        ...
        if (controller.right && gamerunning){
            this.x += this.xSpeed;
        }

        if (controller.left && gamerunning){
            this.x -= this.xSpeed; 
        }
...
class Obstacle {
    constructor(){
        ...
        this.height = 15; 
        this.x = buffer.canvas.width * Math.random();
        this.y = 0;
        this.xSpeed = 0;
        this.ySpeed = 3;
...
let ObstacleWidth = Math.floor(Math.random() * (160) + player1.height);
let obstacles = [];
let frameCounter = 0; 
...
function loop(){
    ...
    ObstacleWidth = Math.floor(Math.random() * (100 - player1.height) + player1.height);
    resize(); 
    background1.clear(); 
    background1.draw();
    player1.draw(); 
